      ...
      <div className="row p-4 ">
        <div className="col-md-4 p-4 border-right">
          <div className="border-top p-2">
            ...
            <h5>
              User : {singleUserOrders ? singleUserOrders.userId.username : ""}{" "}
            </h5>
            <h6>
              Email : {singleUserOrders ? singleUserOrders.userId.email : ""}
            </h6>
          </div>
          <div className="border-top p-2 ">
            ...
            <h6>
              Contact :{" "}
              {singleUserOrders
                ? JSON.parse(singleUserOrders.shippingDetails).phone
                : ""}
            </h6>
            <h6>
              Address :{" "}
              {singleUserOrders
                ? JSON.parse(singleUserOrders.shippingDetails).address
                : ""}
            </h6>
            <h6>
              Address 2 :{" "}
              {singleUserOrders
                ? JSON.parse(singleUserOrders.shippingDetails).address2
                : ""}
            </h6>
            <h6>
              City :{" "}
              {singleUserOrders
                ? JSON.parse(singleUserOrders.shippingDetails).city
                : ""}
            </h6>
            <h6>
              State :{" "}
              {singleUserOrders
                ? JSON.parse(singleUserOrders.shippingDetails).state
                : ""}
            </h6>
          </div>
          <div className="border-top p-2 ">
            ...
            <h6 className="font-weight-bold">
              Total: $
              {singleUserOrders
                ? JSON.parse(singleUserOrders.orderdProducts)
                    .reduce(
          ...
          <div className="p-2">
            {singleUserOrders ? (
              ...
              >
                <option
                  selected={
                    singleUserOrders.orderStatus == "pending" ? true : false
                  }
                  value={"pending"}
                >
                  pending
                ...
                <option
                  selected={
                    singleUserOrders.orderStatus == "packing" ? true : false
                  }
                  value={"packing"}
                >
                  packing
                ...
                <option
                  selected={
                    singleUserOrders.orderStatus == "shipped" ? true : false
                  }
                  value={"shipped"}
                >
                  shipped
                ...
                <option
                  selected={
                    singleUserOrders.orderStatus == "deliverd" ? true : false
                  }
                  value={"deliverd"}
                >
                  deliverd
